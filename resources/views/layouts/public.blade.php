    ...
    <div class="max-w-7xl mx-auto px-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            ...
            <div>
                ...
                <div class="space-y-2.5 text-xs text-purple-200/90 dark:text-slate-300">
                    @php
                        $cAddress = \App\Models\AppSetting::get('contact_address');
                        $cEmail = \App\Models\AppSetting::get('contact_email');
                        $cPhone = \App\Models\AppSetting::get('contact_phone');
                        $cWa = \App\Models\AppSetting::get('contact_whatsapp');
                        $cFb = \App\Models\AppSetting::get('contact_facebook');
                        $cIg = \App\Models\AppSetting::get('contact_instagram');
                        $cYt = \App\Models\AppSetting::get('contact_youtube');
                        $cTt = \App\Models\AppSetting::get('contact_tiktok');
                    @endphp
                    ...
                    <p class="flex items-start space-x-2">
                        ...
                        <span class="leading-relaxed">{{ $cAddress }}</span>
                    </p>
                    @endif
                    @if($cEmail)
                    ...
                    {{-- Social Media Icons --}}
                    @if($cWa || $cFb || $cIg || $cYt || $cTt)
                    <div class="flex items-center gap-3 mt-4 pt-2">
                        @if($cWa)
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $cWa) }}" target="_blank" class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-green-500 hover:text-white transition-all" title="WhatsApp">
                            <i class="fa-brands fa-whatsapp"></i>
                        </a>
                        @endif
                        @if($cFb)
                        <a href="{{ $cFb }}" target="_blank" class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all" title="Facebook">
                            <i class="fa-brands fa-facebook-f"></i>
                        </a>
                        @endif
                        @if($cIg)
                        <a href="{{ $cIg }}" target="_blank" class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-pink-600 hover:text-white transition-all" title="Instagram">
                            <i class="fa-brands fa-instagram"></i>
                        </a>
                        @endif
                        @if($cYt)
                        <a href="{{ $cYt }}" target="_blank" class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-red-600 hover:text-white transition-all" title="YouTube">
                            <i class="fa-brands fa-youtube"></i>
                        </a>
                        @endif
                        @if($cTt)
                        <a href="{{ $cTt }}" target="_blank" class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-gray-800 hover:text-white transition-all" title="TikTok">
